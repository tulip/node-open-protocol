...
class LinkLayer extends Duplex {
    ...
    if (length > 9979) {
      ...
      if (parts > 9) {
        this.emit(
          "error",
          new Error(
            `[LinkLayer] number of messages > 9, MID[${data.mid}], length buffer [${length}]`
          )
        );
  ...
  _onDataOpParser(data) {
    ...
    if (this.linkLayerActive) {
      if (
        this.lastMessageReceived.mid === data.mid &&
        this.lastMessageReceived.sequenceNumber === data.sequenceNumber
      ) {
        duplicateMsg = true;
    ...
    if (data.messageParts !== 0 || this.receiverMessageInParts !== 0) {
      ...
      if (data.messageNumber !== this.numberMessageReceived) {
        ...
        this.emit(
          "error",
          new Error(
            `[LinkLayer] inconsistency message number, MID[${data.mid}]`
          )
        );
        debug(
          "LinkLayer _onDataOpParser err_inconsistency_message_number",
          data
        );
      ...
      if (this.receiverMessageInParts === this.numberMessageReceived) {
        ...
        if (!duplicateMsg) {
          if (
            this.disableMidParsing[data.mid] &&
            data.mid !== POSITIVE_ACK &&
            data.mid !== NEGATIVE_ACK
          ) {
            if (!this.push(data)) {
              this.stream.pause();
              return;
        ...
        if (data.mid === POSITIVE_ACK || data.mid === NEGATIVE_ACK) {
          if (data.sequenceNumber !== this.sequenceNumber) {
            this.emit(
              "error",
              new Error(
                `[LinkLayer] sequence number invalid, MID[${data.mid}], received[${data.sequenceNumber}], expected[${this.sequenceNumber}]`
              )
            );
            debug(
              "LinkLayer _onDataOpParser err_sequence_number_ack_invalid",
              data.mid,
              data.sequenceNumber,
              this.sequenceNumber
            );
          ...
          if (this.sequenceNumberPartner) {
            ...
            if (data.sequenceNumber !== this.sequenceNumberPartner + 1) {
              ...
              this.emit(
                "error",
                new Error(
                  `[LinkLayer] sequence number invalid, MID[${data.mid}]`
                )
              );
              debug(
                "LinkLayer _onDataOpParser err_sequence_number_invalid",
                data.mid,
                data.sequenceNumber,
                this.sequenceNumberPartner
              );
              return;
            }
          }
  ...
  _receiverLinkLayer(data) {
    ...
    if (
      ...
      let err = new Error(
        `incorrect fields of MID, MID[${data.payload.midNumber}] - Error code [${data.payload.errorCode}] -` +
      ...
      if (this.callbackWrite) {
        ...
        debug(
          "LinkLayer _receiverLinkLayer err-incorrect_fields_MID",
          this.message.mid,
          this.sequenceNumber
        );
        this.emit("error", err);
